...
export default function TuitionDetailsPage() {
  ...
  const fetchTuitionDetails = async () => {
    try {
      ...
      // Fetch class logs for current month
  ...
  const fetchFeeRecords = async () => {
    try {
      ...
      console.error("Error in fetchFeeRecords:", error as Error)
    }
  }
  ...
  const addClassLog = async () => {
    try {
      ...
      const { data: userData } = await supabase.from("users").select("id").eq("clerk_id", user?.id).single()
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/10">
      <div className="container mx-auto p-4 md:p-8 pt-16 md:pt-8">
        ...
        <div className="flex flex-wrap gap-3 mb-8 justify-end">
          ...
          <Card className="relative overflow-hidden border-0 shadow-md hover:shadow-lg transition-all duration-300 min-w-[140px]">
            ...
            <CardContent className="p-4 relative">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-lg font-bold text-red-600">{formatCurrency(stats.unpaidFees)}</div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {/* Students Tab - Enhanced with Edit/Delete functionality */}
          ...
          {/* Enhanced Calendar Tab with class logs */}
          ...
          {/* Enhanced Payments Tab with month selection */}
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            >
                              {feeRecord?.status === "paid" ? "Paid" : "Unpaid"}
                            </Badge>
                          </div>
                        </div>
                      )
              ...
              <CardContent className="space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    className="w-full justify-start bg-gradient-to-r from-background to-accent/5 border-border/50 hover:from-accent/10 hover:to-accent/20"
                    onClick={() => {
                      toast.info("Duplicate tuition functionality coming soon")
                    }}
                  >
                ...
                <div className="border-t pt-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        toast.error("Permanent delete functionality will be available in archive section")
                      }}
                    >
...
//   const fetchFeeRecords = async () => {
//     try {
//       // First, create fee records for students who don't have them for the selected month/year
//       const studentsWithoutFees = students.filter((student) => !feeRecords.some((fee) => fee.student_id === student.id))
...
//       // Now fetch all fee records for the selected month/year
//       const { data: feeData, error: feeError } = await supabase
//         .from("fee_records")
//         .select(`
//           *,
...
//       // Update unpaid fees stat
//       const unpaidAmount = formattedFeeRecords
//         .filter((record) => record.status === "due")
//         .reduce((sum, record) => sum + record.amount, 0)

//       setStats((prev) => ({ ...prev, unpaidFees: unpaidAmount }))
//     } catch (error) {
...
//         topic_covered: classLogForm.topic_covered.trim() || null,
//         notes: classLogForm.notes.trim() || null,
//         created_by: userData.id,
//       })
...
//     if (
//       !confirm(
//         "Are you sure you want to archive this tuition? It will be moved to the archive and can be restored later.",
//       )
//     ) {
//       return
//     }
...
//       toast.success("Tuition has been archived")
//       router.push("/tuitions")
//     } catch (error) {
//       console.error("Error archiving tuition:", error)
//       toast.error("Failed to archive tuition")
