// lib/supabase.ts - Fixed version with proper Clerk integration
...
// Create base client for non-authenticated operations
...
export const useSupabase = () => {
  ...
  const authenticatedSupabase = useMemo(() => {
    if (!isLoaded || !userId) {
      return supabase // Return base client if not authenticated
    }
...
export const debugAuthToken = async (getToken: () => Promise<string | null>) => {
  try {
    const token = await getToken({ template: 'supabase' })
    if (token) {
      // Decode JWT to see contents (for debugging only)
